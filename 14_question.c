...
int main() {
    ...
    // Input the attendance matrix
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 7; j++) {
            scanf("%d", &A[i][j]);
            studentTotal[i] += A[i][j];   // Row sum
            dayTotal[j] += A[i][j];       // Column sum
        }
    }
    ...
    // (a) Total number of days each student was present
    printf("Total Present Days:\n");
    for (int i = 0; i < 10; i++) {
        printf("Student %d: %d\n", i + 1, studentTotal[i]);
    }

    // (b) Student with highest attendance
    ...
    // (c) Day with lowest overall attendance
    int minDayAttendance = dayTotal[0];
    int minDay = 1;

    for (int j = 1; j < 7; j++) {
        if (dayTotal[j] < minDayAttendance) {
            minDayAttendance = dayTotal[j];
            minDay = j + 1;
        }
    }
